...
\begin{definition}{The support of a distribution}{supp}
...
\[D^{\alpha}T_f(\varphi) = D^{\alpha}\int_{\Omega}f\varphi\]
...
\begin{proof}
    \begin{enumerate}
        ...
        \[\text{By 1. , }  =\lim_{h\rightarrow0}(T\ast\varphi)(x)-T\ast(\tau_{he_k}\varphi)(x) \]
...
\begin{lemma}{$\testf$ is invariant by convolution with the $\delta$-distribution}{}
    Let $\varphi\in\testf$, then:

    \[\delta\ast\varphi(x) = \varphi(x)\]

\end{lemma}
\begin{proof}
    Simply by computation:

    \[\delta\ast\varphi(x) = \delta(\tau_x\check{\varphi}) = \delta(\varphi(x-y)) = \varphi(x-0) = \varphi(x)\]

\end{proof}

\subsection{Convolution between two distributions}

\begin{definition}{Convolution between two distributions}{convdist}
    Let $T,G\in\dist$, $G$ with compact support, then we define:

    \[T\ast G(\varphi) = T\ast(G\ast\varphi)\]
\end{definition}
