...
\begin{definition}{Test functions}
    aLet $\Omega$ be a subset of $\mathbb{R}^n$. Then $\testf$ is the set of all $C^\infty$ functions defined on $\Omega$ with compact support. 
\end{definition}
...
For any $\varphi$ on this set and any locally integrable function $f:\mathbb{R}^n\rightarrow\mathbb{R}$, using integration by parts we get:
...
\begin{definition}{Distributions}
    aA linear functional $T$ is said to be a distribution if for all sequences $\{\varphi_n\}_{n\in\mathbb{N}}$ in $\testf$ convergent to $\varphi$, the sequence $\{T\varphi_n\}_{n\in\mathbb{N}}$ converges to $T\varphi$ in $\testf$.
\end{definition}
...
\begin{definition}{Distribution given by a function}
    aLet $f:\mathbb{R}^n\rightarrow\mathbb{R}$ be a locally integrable function, we say that the distribution given by $f$ is:
...
\begin{definition}{The Dirac distribution}
    aFor any $\varphi\in\testf$:
